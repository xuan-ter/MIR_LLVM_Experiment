...
TOKIO_DID_INTERACTION_CSV = r"d:\MIR_LLVM\mir_-llvm\tokio\analysis\did\interaction_results.csv"
...
def _edges_from_did_proxy(interaction_csv: str, *, top_n: int) -> List[Dict[str, object]]:
    ...
    df = df.sort_values("abs_delta", ascending=False).head(int(max(1, top_n)))
    edges = []
    for _, r in df.iterrows():
        edges.append(
            {
                "Source": str(r["mir_pass"]).strip(),
                "Target": str(r["llvm_pass"]).strip(),
                "Type": "Interaction",
                "Weight": float(r["delta"]),
                "Stability": float(1.0 if bool(r.get("significant", False)) else 0.5),
            }
        )
    return edges
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if args.proxy != "force" and HAVE_PANDAS and HAVE_SKLEARN and os.path.exists(args.input):
        ...
        X, y, feature_names, mir_passes, llvm_passes = _build_feature_matrix(df, args.metric)
    ...
    if use_proxy and os.path.exists(TOKIO_DID_INTERACTION_CSV):
        proxy_edges = _edges_from_did_proxy(TOKIO_DID_INTERACTION_CSV, top_n=int(args.proxy_top_n))
        if args.proxy == "force":
            edges = proxy_edges
        else:
