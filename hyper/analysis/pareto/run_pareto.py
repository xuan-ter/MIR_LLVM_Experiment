...
HYPER_LASSO_EDGES_CSV = r"d:\MIR_LLVM\mir_-llvm\hyper\analysis\lasso\coupling_edges.csv"
...
def _plot_hv_svg(out_path: str, series: Dict[str, List[Tuple[float, float]]], hv_ref: float, title: str) -> None:
    ...
    parts.append(
        f'<text x="{width/2:.1f}" y="28" text-anchor="middle" font-size="18" font-family="Arial">{_svg_escape(title)}</text>'
    )
    ...
    if hv_ref > 0:
        yref = ty(hv_ref)
        parts.append(
            f'<line x1="{pad_l}" y1="{yref:.2f}" x2="{pad_l+plot_w}" y2="{yref:.2f}" stroke="#333" stroke-dasharray="6,4" stroke-width="1"/>'
        )
        parts.append(
        ...
        y95 = ty(0.95 * hv_ref)
        parts.append(
            f'<line x1="{pad_l}" y1="{y95:.2f}" x2="{pad_l+plot_w}" y2="{y95:.2f}" stroke="#666" stroke-dasharray="3,4" stroke-width="1"/>'
        )
        parts.append(
    ...
    for k in keys:
        ...
        parts.append(
            f'<path d="M {tx(pts[0][0]):.2f} {ty(pts[0][1]):.2f} {d}" fill="none" stroke="{color[k]}" stroke-width="2"/>'
        )
    ...
    parts.append(
        f'<text x="18" y="{pad_t+plot_h/2:.1f}" text-anchor="middle" font-size="14" font-family="Arial" transform="rotate(-90 18 {pad_t+plot_h/2:.1f})">Hypervolume</text>'
    )
...
def _write_graph_stats(out_dir: str) -> None:
    edges = _read_edges(HYPER_LASSO_EDGES_CSV)
    if not edges:
        return
    ...
    for p, deg in degree.items():
        side = "MIR" if p in set(mir_nodes) else "LLVM"
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    ap.add_argument("--input", default=HYPER_EXPERIMENT_CSV)
